...
def _test(epochs):
    ...
    history = model.fit(train_data, train_targets, epochs=epochs,
                        validation_data=(test_data, test_targets), batch_size=1, verbose=0)
...
def cross_check(k_list,epochs):
    ...
    for k in k_list:
        ...
        for i in range(0, k):
            ...
            model = build_model()
            history = model.fit(partial_train_data, partial_train_targets, epochs=epochs, batch_size=1, verbose=0,validation_data=(val_data, val_targets))
            loss = history.history['loss']
            mae = history.history['mean_absolute_error']
            v_loss = history.history['val_loss']
            v_mae = history.history['val_mean_absolute_error']
